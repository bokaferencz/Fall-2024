...
# Identify observations with Cook's distance greater than 4/n (rule of thumb)
...
ggplot(residuals_vs_fitted_df, aes(x = fitted_values, y = residuals)) +
  ...
  geom_smooth(method = "lm", color = "red", se = FALSE) +  # Optional: Add a linear trend line
...
#I decided to keep influential outliers because they might be important to understand in this context, and they don't have an effect on the assumptions
# Complex model with additional predictors
...
```{r}
# Calculate and check the VIF for the new complex model
...
# Final-complex  model with multiple predictors
...
#### The simple model explained a small proportion of variance in pain (Radj2=0.0685), with a significant F-statistic (𝐹(2,155)=6.78, 𝑝<.001. The more complex model, however, accounted for a larger proportion of variance in pain ( and significantly improved the model fit (Radj2 =0.32, F(6,151)= 13.52, p<.001 ).
